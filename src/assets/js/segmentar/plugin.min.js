tinymce.PluginManager.add('tma_segmentar', function(editor, url) {
            ...
            if (selectedTextLength > 0 || node.className == 'annotation') {
                ...
                editor.windowManager.open({
                    ...
                    },
                    {
                        ...
                        label  : 'Color',
                        items  : [
                          { text: ' ', style:'background-color: #0000FF', onclick: updateColorValue("#0000FF")},
                          { text: ' ', style:'background-color: #FF0000', onclick: updateColorValue("#FF0000")}
                        ]
                    },  
                    {
                        type: 'listbox',
                    ...
                    onsubmit: function(e) {
                        if (e.data.annotation) {
                            var dataAnnotation = e.data.annotation; //aqui se guarda la descripcion de la anotacion
                            var color = cColor
                            ...
                            if (anim=='subrayar'){
                                var segment =' <span class="annotation" style="display: line;" id="'+ id +'" data-author="' + /*TMA.author*/ 'Autor' + '"> <a class= "fun-hover">' + selectedText + '</a></span>';
                            }
                            else {
                                var segment =' <span class="annotation" style="display: block;" id="'+ id +'" data-author="' + /*TMA.author*/ 'Autor' + '" style="background-color: '+color+' ">' + selectedText + '</span>';
                            }
                            if ($(node).attr("data-annotation")) {
                                editor.dom.remove(node);
                            }
                            editor.selection.setContent(segment);
                            editor.fire('submitcomment', {id, dataAnnotation, anim,color})
                        } else {
                            editor.windowManager.alert(' Seleccione un texto a segmentar');
                            return false;
                        }
        ...
        editor.addMenuItem('tma_segmentar', {
            text: 'Crear comentario', // TMA.tooltips.annotation_create,
            image: tinyMCE.baseURL + '/img/annotation.png',
            onclick: addComment
        });
        ...
        editor.addMenuItem('tma_segmentardelete', {
            text: 'Eliminar comentario', //TMA.tooltips.annotation_delete,
            image: tinyMCE.baseURL + '/img/annotation-delete.png',
            onclick: deleteComment
        });
